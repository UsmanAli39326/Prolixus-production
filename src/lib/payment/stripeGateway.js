...
function StripeGatewayProvider({ publishableKey, amount, currency, isSubscription, cartItems, children }) {
    ...
    // Show a loader while fetching the clientSecret
    if (!clientSecret) {
        return (
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (error) {
                ...
                let gatewayCustomerId = formData.gatewayCustomerId || null;

                const hasSub = (cartItems || []).some(item => item.purchaseType === "subscribe");

                if (hasSub && (formData.email || gatewayCustomerId)) {
                    try {
                        ...
                        console.log("[StripeCheckoutForm] setup-customer response:", customerData);
                        if (customerData.customerId) {
                            gatewayCustomerId = customerData.customerId;
                        }
                    } catch (subSetupErr) {
                        console.warn("[StripeCheckoutForm] Customer setup notice:", subSetupErr);
                    }
                }

                // Build complete order payload with extracted payment details
                const payload = buildGuestOrderPayload(
                    formData,
                    cartItems,
                    currency,
                    { paymentMethodId: pmId, gatewayCustomerId }
                );
                payload.paymentMethod = "Stripe";
                payload.paymentToken = paymentIntent.id;

                // Ensure createSubscriptions items carry non-null paymentMethodId and gatewayCustomerId
                if (payload.createSubscriptions && payload.createSubscriptions.length > 0) {
                    payload.createSubscriptions = payload.createSubscriptions.map(sub => ({
                        ...sub,
                        paymentMethodId: pmId || sub.paymentMethodId,
                        gatewayCustomerId: gatewayCustomerId || sub.gatewayCustomerId,
                    }));
                }
